...
// Start begins relay operations and starts the WebTransport listener.
func (rr *ReceivingRelay[T]) Start(ctx context.Context) error {
	atomic.StoreInt32(&rr.configFrozen, 1)
	rr.startOutputFanout()

	rr.logKV(types.InfoLevel, "WebTransport receiving relay starting",
		"event", "Start",
		"result", "SUCCESS",
	)
...
func (rr *ReceivingRelay[T]) Stop() {
	rr.logKV(types.InfoLevel, "WebTransport receiving relay stopping",
		"event", "Stop",
		"result", "SUCCESS",
	)
...
// Listen starts the WebTransport listener on the configured address.
func (rr *ReceivingRelay[T]) Listen(listenForever bool, retryInSeconds int) error {
	...
	for {
		...
		if err := server.serve(rr.ctx); err != nil {
			if !listenForever {
				return err
			}
			time.Sleep(time.Duration(retryInSeconds) * time.Second)
...
func (rr *ReceivingRelay[T]) setServer(s *wtServer[T]) {
	rr.serverMu.Lock()
	rr.server = s
	rr.serverMu.Unlock()
}
