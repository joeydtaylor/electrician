...
// ForwardRelay streams payloads to downstream receiving relays over WebTransport.
...
// NewForwardRelay constructs a WebTransport forward relay with optional configuration.
func NewForwardRelay[T any](ctx context.Context, options ...types.Option[*ForwardRelay[T]]) *ForwardRelay[T] {
	...
	fr := &ForwardRelay[T]{
		...
		componentMetadata: types.ComponentMetadata{
			...
			Type: "WEBTRANSPORT_FORWARD_RELAY",
		},
...
func (fr *ForwardRelay[T]) Stop() {
	fr.logKV(types.InfoLevel, "WebTransport forward relay stopping",
		"event", "Stop",
		"result", "SUCCESS",
	)
	fr.cancel()
	fr.closeAllSessions("shutdown")
